...
public final class TinyVideoPlayer: NSObject, TinyVideoPlayerProtocol, TinyLogging, @unchecked Sendable {    
    ...
    public weak var delegate: (any TinyPlayerDelegate)?
    ...
    public override init() {
        ...
        #if os(iOS)
        mediaRouteManager.setDelegate(self)
        #endif
    }
    ...
    fileprivate func attachTimeObserver() {
        ...
        let interval = CMTimeMakeWithSeconds(Float64(TinyVideoPlayerDefaults.timeObservationInterval), preferredTimescale: CMTimeScale(NSEC_PER_SEC))
        ...
        if context == &TinyVideoPlayerAVPlayerObservationContext {
            ...
            if #available(iOS 10.0, tvOS 10.0, *) {
                ...
                if keyPath == #keyPath(AVPlayer.timeControlStatus) {
                    ...
                    @unknown default:
                        fatalError("Update to consider recently added cases in AVPlayer.timeControlStatus")
                    }
                }
            }
            ...
            if keyPath == #keyPath(AVPlayerItem.status) {
                ...
                if let statusCode = change?[.newKey] as? NSNumber {
                    status = AVPlayerItem.Status(rawValue: statusCode.intValue)!
                } else {
                    status = .unknown
                }
    ...
    /**
      ...
      */
    @MainActor fileprivate func updatePlaybackState(_ newState: TinyPlayerState) {
     ...
     */
    @MainActor fileprivate func playerItemIsReadyForPlaying(playerItem: AVPlayerItem) {
